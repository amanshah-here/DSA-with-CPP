...
void KthHelper(Node* root, int K, int currLevel, vector<int>& ans) {    // O(n)
    if (root == NULL) return;
    if (currLevel == K) {
        ans.push_back(root->data); // collect value
        cout << root->data << " "; // also print for quick output
    }

    // recurse to children increasing level
    KthHelper(root->left, K, currLevel + 1, ans);
    KthHelper(root->right, K, currLevel + 1, ans);
}
vector<int> KthLevel(Node* root, int K) {
    vector<int> ans;
    KthHelper(root, K, 1, ans); // start levels at 1 for the root
...
int main() {
    ...
    KthLevel(root, 2); // print values at level 2
